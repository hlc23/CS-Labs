...
def upload_payload():
    ...
    if request.method == 'GET':
        return render_template('upload_payload.html')

    # POST: handle file upload
    if 'file' not in request.files:
        return render_template('upload_payload.html', error='No file part in the request.'), 400

    file = request.files['file']
    if file.filename == '':
        return render_template('upload_payload.html', error='No file selected.'), 400

    if not _allowed_file(file.filename):
        return render_template('upload_payload.html', error='Only .html or .htm files are allowed.'), 400

    filename = secure_filename(file.filename)
    if not filename:
        return render_template('upload_payload.html', error='Invalid filename.'), 400

    save_path = os.path.join(UPLOAD_FOLDER, filename)
    try:
        file.save(save_path)
    except Exception as e:
        return render_template('upload_payload.html', error=f'Failed to save file: {e}'), 500

    # Show success message on the upload page with a link to the payload
    payload_url = url_for('serve_payload', filename=filename)
    return render_template('upload_payload.html', success='Upload successful.', payload_url=payload_url)
